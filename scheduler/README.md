...
export DATABASE_URL=postgres://postgres:postgres@localhost:5432/bitcoin
python main.py

# simulate the scheduler
curl -X POST http://localhost:8080/tasks/fetch-price
```

## Deploy (Cloud Run + Cloud Scheduler)

```bash
gcloud run deploy bitcoin-scheduler \
  --source . \
  --set-env-vars DATABASE_URL=postgres://... \
  --no-allow-unauthenticated

# fire every 5 minutes, authenticating with OIDC
gcloud scheduler jobs create http fetch-bitcoin-price \
  --schedule "*/5 * * * *" \
  --uri "https://<service-url>/tasks/fetch-price" \
  --http-method POST \
  --oidc-service-account-email <sa>@<project>.iam.gserviceaccount.com
```

Note: CoinGecko's free tier rate-limits per IP; a schedule of once a minute
or slower is safe.
