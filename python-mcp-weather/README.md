...
Python MCP server, one tool: `get_weather(city)`. Backed by free Open-Meteo API (no key needed). Serve over Streamable HTTP at `/mcp`, run as container, deploy to Cloud Run. See `mcp_weather/README.md` for run/deploy steps.
